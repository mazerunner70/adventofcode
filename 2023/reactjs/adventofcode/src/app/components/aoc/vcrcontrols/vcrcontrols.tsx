...
export const controlType = {
  PLAY: {
    name: "play",
    icon: playButton,
    isActive: (speed: number) => speed == 1,
  },
  PAUSE: {
    name: "pause",
    icon: pauseButton,
    isActive: (speed: number) => false,
  },
  STOP: {
    name: "stop",
    icon: stopButton,
    isActive: (speed: number) => speed == 0,
  },
  RECORD: {
    name: "record",
    icon: recordButton,
...
export const VCRControls = ({
  ...
  const rewindHandler = () => {
    if (speedState >= 0) onSpeedChange(-1);
    else onSpeedChange(speedState * 2);
  };
