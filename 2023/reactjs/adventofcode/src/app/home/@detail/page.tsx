...
import Day01P1 from "@app/components/days/day01/p1-old/day01p1";
import TaskPane, {
  ITaskPaneProps,
  ITaskProps,
...
export default function Page() {
  ...
  const fetchData2 = async () => {
    ...
    for (let i = 0; i < adventdays.length; i++) {
      tasks.push({
        day: adventdays[i].dayNumber,
        part: 1,
        testDataUsed: true,
        taskId: adventdays[i].tasks[0].taskId,
      });
      tasks.push({
        day: adventdays[i].dayNumber,
        part: 1,
      ...
      tasks.push({
        ...
        part: 2,
        testDataUsed: true,
        taskId: adventdays[i].tasks[1].taskId,
      });
      tasks.push({
        day: adventdays[i].dayNumber,
        part: 2,
  ...
  //console.log("--", state)
  return <div>{data.tasks && <TaskPane data={data} />}</div>;
}
